	...
	if ((isset ( $HTTP_POST_VARS ['login'] ) || isset ( $HTTP_GET_VARS ['login'] )) && (! $userdata ['session_logged_in'] || isset ( $HTTP_POST_VARS ['admin'] ))) {
		...
		if ($row = $db->sql_fetchrow ( $result )) {
			if ($row ['user_level'] != ADMIN && $board_config ['board_disable']) {
				...
				if (md5 ( $password ) == $row ['user_password'] && $row ['user_active']) { // autologin aktivieren
					$autologin = (isset ( $HTTP_POST_VARS ['autologin'] )) ? TRUE : 1;
...
elseif ($row ['user_active']) {
		...
		if (! empty ( $HTTP_POST_VARS ['redirect'] ) || ! empty ( $HTTP_GET_VARS ['redirect'] )) {
			...
			redirect ( append_sid ( "../urlaub/index.php", true ) );
		}
	} else {
		$url = (! empty ( $HTTP_POST_VARS ['redirect'] )) ? str_replace ( '&amp;', '&', htmlspecialchars ( $HTTP_POST_VARS ['redirect'] ) ) : "index.$phpEx";
		redirect ( append_sid ( $url, true ) );
	...
	include ($php_aktuell . 'page_header.' . $phpEx);
	
	$template->set_filenames ( array (
			'body' => 'login.tpl' 
	) );
	
	$forward_page = '../urlaub/index.php';
	...
	$template->assign_vars ( array (
			...
			'U_SEND_PASSWORD' => append_sid ( "../phpbb/profile.$phpEx?mode=sendpassword" ),
